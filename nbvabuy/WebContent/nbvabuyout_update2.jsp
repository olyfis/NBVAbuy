...
<%
	...
	//String color_err = "#EC7063";
	
	String color_err = "#D7748B";
...
public void  buildCellsContract( JspWriter out, ContractData contract, String formUrlDisp, int mthRem  ) throws IOException {
	...
	*/
	...
	//System.out.println("*** compareDate returned=" + rVal + "--   (Results: (0-> equalTo) -- (1-> OK) -- (-1 -> Error))");
		...
		if (  ! contract.getPurOption().equals("02"))  {
			//System.out.println("***  Error: Purchase Option should be: 01");
			out.println( "<td class=\"a\">"  + contract.getPurOption() + "</td></tr>");
		} else {
			out.println( "<td   bgcolor=\" + color_err + \"  class=\"a\">  <B>*** ERROR  - Should be 01 ->"  + contract.getPurOption() + "</B></td></tr>");
		}
...
public String  buildCellsAsset( HashMap<String, String> hm, JspWriter out,  List<Pair<ContractData, List<AssetData> >> rtnPair, String opt, String dispFlag,  HashMap<String, String> cMap,  HashMap<String, String> rMap ) throws IOException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
	...
	if (listArrSZ > 0) {
		...
		cells +="  <form name=\"getAsset\"  method=\"POST\" action=\"/nbvabuy/nbvabuymod\" > ";
		...
		for (int i = 0; i < listArrSZ; i++ ) {
			...
			String aID = "";
				for (int j = 0; j < rtnArrSZ; j++) {
					...
					if (asset.getDispCode() == 1) {
						code = "Buyout";
						dCode2 = 1;
					} /*else if (asset.getDispCode() == 0) {
					...
					*/
					...
					cells += "<TD>BuyOut</td> ";
					//System.out.println("*** !!!! *** Asset=" + aID + "-- **DCODE=" +dCode2 + "-- Code="  + code  + "--" );
...
/*
					...
					cells += "<option value=\"" + dCode2 + "\" selected    > " + code + " </option>  ";
...
<%
	...
	String filePath2 = "C:\\Java_Dev\\props\\headers\\NBVA\\NBVA_AssetHrdCode.txt";
